...
import { getSupabaseConfig } from "../lib/api/supabase.functions";
...
export type Sync = {
  status: SyncStatus;
  configSource: "server" | "local" | "none" | "loading";
  email: string | null;
  linkSent: boolean;
  errorMsg: string;
...
export function useSync(
  ...
  // Resolve config: deploy-time secrets win, then a locally pasted config.
  useEffect(() => {
    let live = true;
    (async () => {
      let server: SupabaseConfig | null = null;
      try {
        server = (await getSupabaseConfig()) as SupabaseConfig | null;
      } catch {
        server = null;
      }
      if (!live) return;
